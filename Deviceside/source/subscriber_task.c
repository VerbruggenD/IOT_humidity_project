...
void subscriber_task(void *pvParameters)
{
    ...
    cyhal_gpio_init(P12_0, CYHAL_GPIO_DIR_OUTPUT,
    ...
    while (true)
    {
        ...
        if (pdTRUE == xQueueReceive(subscriber_task_q, &subscriber_q_data, portMAX_DELAY))
        {
            switch(subscriber_q_data.cmd)
            {
                ...
                {
                    if((subscriber_q_data.data == 0)||(subscriber_q_data.data == 1)){
						/* Update the Dehumidifier state as per received notification. */
						cyhal_gpio_write(P12_0, !subscriber_q_data.data);
...
void mqtt_subscription_callback(cy_mqtt_publish_info_t *received_msg_info)
{
	...
	const char *received_msg_control;
	int received_msg_control_len;

	/* Received MQTT message */
	if(strncmp(received_msg_info->topic, "sensor/humidity", strlen("sensor/humidity")) == 0){
		char check[received_msg_info->payload_len];
	...
	else if(strncmp(received_msg_info->topic, "sensor/temperature", strlen("sensor/temperature")) == 0){
		char check[received_msg_info->payload_len];
		memcpy(check , received_msg_info->payload, received_msg_info->payload_len);
		int intcheck = atoi(check);
		if(intcheck){
			printf("The current temperature is: %d", intcheck);
			printf("C\n");
			return;
		}
		else{
	...
	else if(strncmp(received_msg_info->topic, "sensor/config", strlen("sensor/config")) == 0){
		char check[received_msg_info->payload_len];
		memcpy(check , received_msg_info->payload, received_msg_info->payload_len);
		int intcheck = atoi(check);
		if(intcheck){
	...
	else if(strncmp(received_msg_info->topic, "sensor/control", strlen("sensor/control")) == 0){
		control = 1;
		received_msg_control = received_msg_info->payload;
		received_msg_control_len = received_msg_info->payload_len;
		//printf("read control werkt\n");
		/* Assign the device state depending on the received MQTT message. */
		if ((strlen(MQTT_DEVICE_ON_MESSAGE) == received_msg_control_len) &&
			(strncmp(MQTT_DEVICE_ON_MESSAGE, received_msg_control, received_msg_control_len) == 0))
		{
			subscriber_q_data.data = DEVICE_ON_STATE;
		}
		else if ((strlen(MQTT_DEVICE_OFF_MESSAGE) == received_msg_control_len) &&
				 (strncmp(MQTT_DEVICE_OFF_MESSAGE, received_msg_control, received_msg_control_len) == 0))
		{
			subscriber_q_data.data = DEVICE_OFF_STATE;
		}
		else if ((strlen(MQTT_DEVICE_AUTO_MESSAGE) == received_msg_control_len) &&
				 (strncmp(MQTT_DEVICE_AUTO_MESSAGE, received_msg_control, received_msg_control_len) == 0))
		{
			control = 0;
			if(!control){
				if(humidity <= treshold){
